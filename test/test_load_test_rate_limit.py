...
from scripts.load_test_rate_limit import build_summary_payload, parse_concurrency_profile, percentile
...
def test_parse_concurrency_profile_returns_profile_values() -> None:
    assert parse_concurrency_profile("5,10,20") == [5, 10, 20]


def test_build_summary_payload_can_include_stage_summaries() -> None:
    args = Namespace(
        base_url="http://127.0.0.1:8080",
        requests=10,
        concurrency=2,
    ...
    payload = build_summary_payload(
        ...
        latency_seconds=[0.1, 0.2, 0.15, 0.12],
        timestamp="20260528T000000Z",
        stage_summaries=[{"concurrency": 2, "success_rate": 0.8, "throttled_rate": 0.2}],
    )

    assert payload["stage_summaries"][0]["concurrency"] == 2
